...
data "aws_ecr_repository" "{{ cookiecutter.project_snake }}" {
  provider = aws.mgmt
  name     = "cko-fort/{{ cookiecutter.project_slug }}"
...
data "aws_iam_role" "{{ cookiecutter.scheme_slug }}_batch_service_role" {
  name = "${local.application_name}-batch-service-role-${terraform.workspace}"
}